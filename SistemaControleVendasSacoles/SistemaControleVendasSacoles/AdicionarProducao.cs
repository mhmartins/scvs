...
{
    public partial class AdicionarProducao : Form
    {
        public AdicionarProducao()
        {
            InitializeComponent();
        }

        private void AdicionarProducao_Load(object sender, EventArgs e)
        {
            // TODO: This line of code loads data into the 'adicionaProd.sacoles' table. You can move, or remove it, as needed.
            this.sacolesTableAdapter.Fill(this.adicionaProd.sacoles);
        ...
        {
            this.label12.DataBindings.Clear();
            ...
            if (!CamposObrigPreenchidos)//se o campos estiver preenchido ele entra 
            {
                ...
                {
                    float total = 0;
                    total = float.Parse(precoLabel1.Text) * int.Parse(tbxQuantAdd.Text);//pega o total liquido
                    string Valtotal = total.ToString("N", new CultureInfo("pt-BR"));
                    Valtotal = ("R$ " + Valtotal).ToString();

                    quantTextBox.Text = (int.Parse(quantTextBox.Text) + int.Parse(tbxQuantAdd.Text)).ToString();//atualiza a quantidade no estoque
                    float custo = float.Parse(tbxLeite.Text) + float.Parse(tbxEssencia.Text) + float.Parse(tbxSaco.Text) + float.Parse(tbxOutros.Text) + float.Parse(tbxValAcucar.Text) + float.Parse(tbxValLiga.Text);
                    ...
                    try
                    {
                        ...
                        this.label12.DataBindings.Add("Text", source, "data", true);
                        //this.textBox1.DataBindings.Add("Text", source, "data", true);

                        if (label12.Text == "")//se a DATA for NOVA
                        {
                        ...
                        else
                        {
                            ...
                            this.label13.DataBindings.Add("Text", source1, "total", true);
                            //MessageBox.Show(label13.Text);
                            //MessageBox.Show("oi" + custo);

                            float soma = (custo + float.Parse(label13.Text));
                        ...
                        string update = "UPDATE sacoles SET quant= '" + quantTextBox.Text + "' WHERE sabor = '" + saborLabel1.Text + "' && tipo = 1 ";
                        MySqlCommand comandos = new MySqlCommand(update, conexao);
